...
<sub>Tampermonkey 用户脚本 · 注入到 chatgpt.com · Session 导出本地处理 · 零依赖 · 中英双语（时区自动 + 可手动切换）· v2.5.3</sub>

<br>

<sub>**ChatGPT Session All-in-One Toolbox** — Session ↔ auth.json / CPA / Sub2API / Cockpit / 9router / AxonHub / Codex-Manager / Codex / Raw (N×N) · Plus/Team single checkout links · zh/en UI · local-only conversion.</sub>

<br>

[![version](https://img.shields.io/badge/版本-2.5.3-ff5722?style=for-the-badge&labelColor=1a1614)](https://github.com/1837620622/chatgpt-specimen-toolbox/releases)
...
[功能特性](#一-功能特性) · [9 种导出](#二-9-种导出格式) · [导入互转](#三-11-种来源导入--n×n-互转) · [订阅链接](#四-订阅链接生成) · [安装](#五-安装方法) · [使用](#六-使用流程) · [安全](#九-安全--隐私--风险) · [FAQ](#十-常见问题) · [联系](#十二-联系方式)
...
| 版本 | 要点 |
|:--|:--|
| **v2.5.3** | 9 种导出对齐 gtxx3600 上游；Session→auth.json 字段对照写清；`sessionToken` 不再误进 `refresh_token`；增加 `tests/export-format-parity.mjs` |
| **v2.5.2** | 安全加固（Stripe 消息桥白名单、支付链 URL 校验、自定义 token 不落盘）；Plus 双语与生成防连点 |
| **v2.5.1** | 代充封控提示 + 成品号引导；移除 Plus 区域批量生成；中英外壳按时区切换 |
| **v2.4.x** | Stripe init 三步法长链 + 广告拦截代理兜底；浏览器扩展版 |
...
（面板「鉴权 · 导出」一键拉取；也可在「导入 · 转换」粘贴同结构 JSON。）

全部对照上游 [`gtxx3600/GPTSession2CPAandSub2API`](https://github.com/gtxx3600/GPTSession2CPAandSub2API) **main 分支 `docs/index.html` 转换逻辑**核对。**上游 schema 更新，本脚本会跟进。**

| 编号 | 格式 | 用途 | 关键字段（对齐上游） |
|:--:|:--|:--|:--|
| 01 | `auth.json`       | Codex CLI 原生（`~/.codex/auth.json`） | `auth_mode` · `OPENAI_API_KEY` · `tokens.{id,access,refresh}_token` · `tokens.account_id` · `last_refresh` |
| 02 | `Codex Auth`      | 重组 id_token 含 email / profile         | 兼容旧版 `codex-auth.js` 输出（本仓库独有互转形态） |
| 03 | `CPA`             | CLI Proxy API 中转                       | `type: "codex"` 平铺 · `session_token` · `id_token_synthetic` 兜底 |
| 04 | `Sub2API`         | CPA2sub2API / 中转 accounts 包           | `exported_at` · `accounts[]` · 无 OAuth refresh 时写 `expires_at`（unix）+ `auto_pause_on_expired` |
| 05 | `Cockpit`         | Cockpit Tools 扁平导入                   | `type: "codex"` 平铺 `id_token/access_token/refresh_token/account_id/email/expired` |
| 06 | `9router`         | 9router Codex OAuth                      | camelCase `accessToken` · `providerSpecificData` · `provider/authType` |
| 07 | `AxonHub`         | AxonHub Codex auth.json                  | 缺 OAuth refresh 时写 `__missing_refresh_token__` 占位 |
| 08 | `Codex-Manager`   | Codex-Manager 批量导入                   | `tokens` + `meta.{label,workspace_id,chatgpt_account_id,note}`；无真实 id_token 则 `""` |
| 09 | `Raw Session`     | 原始 session 输出                        | 不变换，方便调试 |

### 2.1 Session → `auth.json` 字段对照（最常用）

以 `/api/auth/session` 返回为例（字段名以网页实际 JSON 为准）：

| session 来源 | → `auth.json` | 说明 |
|:--|:--|:--|
| `accessToken` | `tokens.access_token` | 原样写入（JWT） |
| `account.id` 或 JWT `https://api.openai.com/auth.chatgpt_account_id` | `tokens.account_id` | 两者通常一致 |
| （Web session **没有** OAuth `id_token`） | `tokens.id_token` | **合成 JWT**（`cpa_synthetic`，claims 含 account / plan / user / email），与上游一致 |
| （Web session **没有** OAuth `refresh_token`） | `tokens.refresh_token` | **空字符串 `""`** |
| `sessionToken` | **不进入 auth.json** | JWE 会话密文；只写入 **CPA** 的 `session_token` 等需要它的格式 |
| `user.email` / JWT profile.email | 合成 id_token 的 `email` claim | auth.json 顶层无单独 email 字段 |
| 导出时刻 | `last_refresh` | ISO 时间；`auth_mode: "chatgpt"` · `OPENAI_API_KEY: null` |
...
```json
{
  ...
  "OPENAI_API_KEY": null,
  "tokens": {
    "id_token": "<synthetic-or-real-jwt>",
    "access_token": "<session.accessToken>",
    "refresh_token": "",
    "account_id": "<session.account.id>"
  },
  "last_refresh": "2026-07-15T00:00:00.000Z"
}
```
...
| 错误做法 | 正确做法 |
|:--|:--|
| 把 `sessionToken` 塞进 `tokens.refresh_token` | `refresh_token` 仅放 **OAuth refresh**；Web session 没有就留 `""` |
| 把 `accessToken` 原样当 `id_token` | 无真实 id_token 时用**合成 JWT**（上游同款），不要直接复制 access |
| 没有 `sessionToken` 就不能导出 auth.json | **可以导出**；auth.json 只强制 `accessToken` + `account_id` |

> ⚠️ `accessToken` / `sessionToken` **等同密码**。不要发到公开群 / Issue / 聊天记录。若已泄露，请在 ChatGPT 退出登录并视情况改密 / 吊起会话。

### 2.2 能力边界（格式正确 ≠ 一定能长期用）

转换结果的 **JSON 形状** 已对照上游与 Codex CLI `auth.json` schema 校验通过，但业务上仍有限制：

| 点 | 说明 |
|:--|:--|
| **无 OAuth refresh** | 网页 session 通常没有可刷新的 `refresh_token`，access **过期后无法自动续** |
| **OpenAI 策略** | 用 Web session 转换导入 Codex / 中转，可能受手机绑定、风控等限制 |
| **更稳妥** | 需要长期可用时，优先官方 `codex login`，或微信购买成品号（备注「购买成品号」· **无无偿服务**） |
| **回归** | 仓库内 `tests/export-format-parity.mjs` 可本地跑形状对照（可选拉上游 `docs/index.html`） |
...
| 06 | **Cockpit**              | 扁平 `type=codex` **或** 历史 `tokens` 嵌套 + `account_id`/`expired` | Cockpit Tools 导出 |
...
> ✓ = 可转换 · ✱ = 源数据缺 OAuth `refresh_token` / 真实 `id_token` 时，目标格式会降级（如 `auth.json` 的 `refresh_token` 写 `""`、AxonHub 写占位、Codex-Manager 的 `id_token` 写 `""`）。**缺 `sessionToken` 不再阻止导出 auth.json**（sessionToken 只进 CPA 等字段）。
...
> 请求体对齐网页 Plus / Team 升级弹窗；v2.4.x 起 Stripe init 三步法长链；**v2.5.x** 起：代充封控提示、中英外壳、**仅单条生成**（后链使前链失效）。

### 4.0 重要说明 · 代充封控与单条生成（v2.5.3）
...
**安全说明**：自定义 token **仅保存在当前页面内存**，**不写 localStorage**（避免同源页面脚本误读）。刷新页面后需重新粘贴。
...
见目录 [`ChatGPT全能助手-扩展版/`](./ChatGPT全能助手-扩展版/) 与其中的 `安装说明.md`。功能与油猴版对齐（v2.5.3：中英双语、代充封控提示、仅单条生成、后链会使前链失效）。

> 装载时选择的是 **`ChatGPT全能助手-扩展版/` 整个目录**（Chrome / Edge）或该目录内的 `manifest.json`（Firefox），**不是**仓库根目录。

### ⚠️ 不要用仓库根目录的遗留单文件脚本

根目录下的 `plus.js` / `gpt-team.js` / `codex-auth.js` 是早期 **F12 控制台粘贴** 原型，**已过时、不再维护**，与 v2.5.3 主工具不对齐：
...
```
   ...
   ║    ChatGPT 全能助手 · 工作台                                  ║
   ║                                                               ║
   ║    v2.5.3  ·  作者 传康KK-CKNB  ·  微信 1837620622  · [中/EN]  ║
   ╟───────────────────────────────────────────────────────────────╢
   ...
   ║                                                               ║
   ╠═══════════════════════════════════════════════════════════════╣
   ║  v2.5.3 · 9 出 × 11 入 · 15 支付区  ⌘⇧K 切换  Esc 关闭        ║
   ╚═══════════════════════════════════════════════════════════════╝
```
...
| 复制微信号 | Plus → 购买说明 →「复制微信号」（加好友请备注「购买成品号」） |
| 导出格式回归测试 | `node tests/export-format-parity.mjs`（可选 `UPSTREAM_HTML=…`） |

---

## 九 · 安全 · 隐私 · 风险

- **Session 转换本地完成**：导出 / 导入互转不上传 token；自定义 token 仅内存、不写 localStorage。
- 但 `accessToken` / `sessionToken` **等同密码**：
  - 别截图发群、别贴 Issue / 公开频道、别提交到 Git
  - 泄露后请退出登录并视情况改密
- **用户主动点「生成支付链接」时**才会请求：
  - ChatGPT 同源 `/backend-api/payments/checkout`
  - Stripe `payment_pages/.../init`（取长链；可被广告拦截时走自有域名代理，**不携带 accessToken**）
- 优惠码字段空着 = 不附带；写错会被服务端 400。
- **请勿批量狂刷 checkout**（后链会使前链失效，也易触发风控）。
...
<details>
<summary><b>auth.json 导出还需要 sessionToken 吗？报缺字段怎么办？</b></summary>

**v2.5.3 起：auth.json 不再依赖 `sessionToken`。**

| 需要 | 说明 |
|:--|:--|
| **必须有** | `accessToken` + 账号 ID（`account.id` 或 JWT 里的 `chatgpt_account_id`） |
| **sessionToken** | **不写进 auth.json**；只进入 CPA 的 `session_token` 等字段。没有它也能导出 auth.json |
| **refresh_token** | Web session 通常没有 OAuth refresh → auth.json 里写 **空串 `""`**（正确，不是 bug） |
| **id_token** | Web session 通常没有 → 写 **合成 JWT**（与上游一致，Codex 只解析 claims 不验签） |

若导出失败，多半是 session 里根本没有 `accessToken`，或 JWT / account 里挖不到 `account_id`。可 F12 打开 `/api/auth/session` 确认 JSON 是否完整。

需要长期可刷新的 Codex 登录时，仍建议官方 `codex login`（会写入真实 OAuth refresh）。
</details>

<details>
<summary><b>导出的 auth.json 格式对，但 Codex / 中转仍登录失败？</b></summary>

**格式正确 ≠ 一定能用。** 常见原因：

1. access token 已过期（Web 转换产物无法自动 refresh）
2. OpenAI 对 Web session / 灰产导入的风控（手机绑定、账号类型等）
3. 目标工具版本要求真实 OAuth 链路

处理：重新在 chatgpt.com 登录后导出；或 `codex login`；或微信购买成品号（备注「购买成品号」· **无无偿服务**）。
</details>
...
**v2.5.2+**：外壳 chrome（标题 / Tab / 页脚 / FAB）、Plus「代充封控」横幅/详情、以及 Plus **区域标签与生成态提示** 支持中英双语。

- **自动**：中文时区（含上海/港/澳/台/新加坡等）→ 中文；否则看浏览器 `zh*` 语言；再否则英文
- **手动**：面板右上角语言按钮切换，写入 localStorage 后优先于时区
- **未全覆盖**：Team / 导入转换 / 部分 toast 仍以中文为主（见路线图）
</details>
...
- [x] 中英双语（外壳 chrome + Plus 代充封控横幅/详情）+ 按时区自动切换（可手动覆盖）—— v2.5.3 已实现
- [x] 代充封控提示改为购买成品号；移除 Plus 区域批量生成（仅单条，后链使前链失效）—— v2.5.1+ 已实现
- [x] 9 种导出格式对齐 gtxx3600 上游 + Session→auth.json 对照文档 + parity 测试 —— v2.5.3 已实现
