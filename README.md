...
<sub>Tampermonkey 用户脚本 · 注入到 chatgpt.com · Session 导出本地处理 · 零依赖 · 中英双语（时区自动 + 可手动切换）· v2.5.1</sub>
...
[![version](https://img.shields.io/badge/版本-2.5.1-ff5722?style=for-the-badge&labelColor=1a1614)](https://github.com/1837620622/chatgpt-specimen-toolbox/releases)
...
| **成品号购买** | 代充已封控：Plus 横幅引导微信 **传康KK（1837620622）** 本地购买成品号 |
...
### 4.0 重要说明 · 代充封控与单条生成（v2.5.1）

| 要点 | 说明 |
|:--|:--|
| **代充已封控** | 低价代充 / 灰产卡密 / 批量试用路径风控严重。**界面已删除旧版「不到 3 元拿下 PLUS」教程横幅**，不再作为功能或教程维护。 |
| **成品号推荐** | 需要稳定可用账号 → 微信 **传康KK（1837620622）** 本地购买成品号。 |
...
见目录 [`ChatGPT全能助手-扩展版/`](./ChatGPT全能助手-扩展版/) 与其中的 `安装说明.md`。功能与油猴版对齐（v2.5.1：中英双语、代充封控提示、仅单条生成、后链会使前链失效）。
...
```
   ...
   ║    ChatGPT 全能助手 · 工作台                                  ║
   ║                                                               ║
   ║    v2.5.1  ·  作者 传康KK-CKNB  ·  微信 1837620622  · [中/EN]  ║
   ╟───────────────────────────────────────────────────────────────╢
   ...
   ║                                                               ║
   ╠═══════════════════════════════════════════════════════════════╣
   ║  v2.5.1 · 9 出 × 11 入 · 15 支付区  ⌘⇧K 切换  Esc 关闭        ║
   ╚═══════════════════════════════════════════════════════════════╝
```
...
**关键认知**：脚本只负责生成 checkout 链接，**支付环节是你与 PayPal/Stripe 直接的事**。链接打开后的行为差异由 PayPal/Stripe 端决定。需要稳定账号时，优先微信 **传康KK（1837620622）** 购买成品号。
</details>
...
- [x] 中英双语（外壳 chrome + Plus 代充封控横幅/详情）+ 按时区自动切换（可手动覆盖）—— v2.5.1 已实现
...
**代充已封控**：低价代充 / 灰产卡密路径风控严重，界面不再提供相关教程。需要稳定可用的 **成品号** 请微信联系 **传康KK（1837620622）** 本地购买。
